...
function ogip_dataset(
    ...
    obs_id = OGIP._fitsfiles_getkey(header, "OBS_ID", "[no observation id]")
    exposure_id = OGIP._fitsfiles_getkey(header, "EXP_ID", "[no exposure id]")
    object = OGIP._fitsfiles_getkey(header, "OBJECT", "[no object]")
